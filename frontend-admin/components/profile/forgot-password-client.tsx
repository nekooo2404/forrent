  ...
  async function handleRequest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        setState({ loading: false, message: "", error: messageFrom(payload, "Không thể gửi email đặt lại mật khẩu.") });
        return;
      }
      setState({ loading: false, message: "Nếu email tồn tại, hướng dẫn đặt lại mật khẩu đã được gửi.", error: "" });
    } catch {
  ...
  return (
    <div className="w-full max-w-md rounded-lg border border-outline-variant/10 bg-surface-container-lowest p-8 shadow-elevated md:p-10">
      <div className="mb-10 text-center">
        <h1 className="mb-3 font-headline-md text-headline-md text-primary">
      ...
      {isConfirmMode ? (
        ...
        <form className="space-y-6" onSubmit={handleRequest}>
          ...
          <Status message={state.message} error={state.error} />
          <SubmitButton loading={state.loading}>Gửi liên kết</SubmitButton>
        </form>
      )}
